...
class category extends Model
{
    ...
    public static function categoryDetails($url){
        ...
        if($categoryDetails['parent_id']==0){
            // Show Main Category in top of the page
            $breadcrumbs = '<li class="breadcrumb-item active" aria-current="page"><a href="'.url($categoryDetails['url']).'">'.$categoryDetails['category_name'].'</a></li>';
        }else{
            // Show Main Category and Sub Category in top of the page
            $parentCategory = Category::select('category_name', 'url')->where('id', $categoryDetails['parent_id'])->first()->toArray();
            $breadcrumbs = '<li class="breadcrumb-item" aria-current="page"><a href="'.url($parentCategory['url']).'">'.$parentCategory['category_name'].'</a></li>
            <li class="breadcrumb-item active" aria-current="page"><a href="'.url($categoryDetails['url']).'">'.$categoryDetails['category_name'].'</a></li>';
        }
